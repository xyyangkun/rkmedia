...
int RgaFilter::Process(std::shared_ptr<MediaBuffer> input,
  ...
  cur_hide = hide;
  std::map<std::string, OsdInfo> cur_osds = osds;
...
int RgaFilter::IoCtrl(unsigned long int request, ...) {
  ...
  case S_RGA_OSD_INFO: {
    ...
    OsdInfo info = {osd->data, osd->pix_fmt};
    char str[128];
    snprintf(str, sizeof(str), "%d, %d, %d, %d", osd->x, osd->y, osd->w,
             osd->h);
    param_mtx.lock();
    osds[str] = info;
    param_mtx.unlock();
...
int rga_blit(std::shared_ptr<ImageBuffer> src, std::shared_ptr<ImageBuffer> dst,
  ...
  for (auto &osd : osds) {
    int x, y, w, h;
    sscanf(osd.first.c_str(), "%d, %d, %d, %d", &x, &y, &w, &h);

    memcpy(&src_info, &dst_info, sizeof(rga_info_t));
    src_info.blend = 0xff0501;
    rga_set_rect(&src_info.rect, x, y, w, h, dst->GetVirWidth(),
                 dst->GetVirHeight(), get_rga_format(dst->GetPixelFormat()));
    memset(&pat_info, 0, sizeof(rga_info_t));
    pat_info.virAddr = osd.second.data;
    pat_info.mmuFlag = 1;
    rga_set_rect(&pat_info.rect, 0, 0, w, h, w, h,
                 get_rga_format(osd.second.fmt));
